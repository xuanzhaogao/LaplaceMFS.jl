# construct the mfs operator for multiple spheres with unknown ordering [p; -q]
function multispheres_G(r::T, r_p::T, M::Int, N::Int, centers::Matrix{T}, eps_r::CT) where {T, CT}
...
function multispheres_G_fmm(
    ...
    D_pr = VT.(mats.D_pr)
    D_qr = VT.(mats.D_qr)
    tmp_pot_self = zeros(VT, mats.M)
    tmp_dn_self = zeros(VT, mats.M)
    ...
    function _mul!(y, x)
        ...
        @inbounds for s in 1:nspheres
            ...
            psrc = (s - 1) * mats.N + 1 : s * mats.N
            mul!(tmp_pot_self, S_pr, view(p, psrc))
            y[pot_trg] .= tmp_pot_self
            y[dn_trg] .= one_minus_eps .* VT.(dn_left[src])
        end
